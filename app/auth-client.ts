...
export function startLogin(returnTo = `${window.location.pathname}${window.location.search}${window.location.hash}`) { window.location.assign(`/auth/login?return_to=${encodeURIComponent(returnTo)}`); }
export async function logout() { await endSSOSession('/api/auth/logout'); window.location.replace('/'); }
