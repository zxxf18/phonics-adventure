...
export async function GET(request: Request) {
  ...
  const url = new URL(request.url); const state = randomToken(); const nonce = randomToken(); const returnTo = safeReturnTo(url.searchParams.get('return_to')); const redirectURL = `${url.origin}/auth/callback`;
